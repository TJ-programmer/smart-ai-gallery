...
import { clearSelection, deleteSelectedPhotos, selectAllPhotos, selectNumSelectedPhotos, setSelectedPhotos, uploadPhotos } from "../../store/photos";
import { useAppDispatch, useAppSelector } from "../../store";
import { userID } from "../../config/app";


import { Button, FloatButton, Upload, UploadFile } from "antd";
import { CheckSquareOutlined, CloseOutlined, CloudUploadOutlined, DeleteOutlined, FolderAddOutlined, SelectOutlined } from "@ant-design/icons";
...
const PhotosPage = () => {
  ...
  const filteredPhotos = useMemo(() => {
    ...
    return Object.fromEntries(
      Object.entries(photos).filter(([, photo]) => {
        const haystack = [
          photo.name,
          photo.description || "",
          ...(photo.tags || []),
  ...
  const createAlbum_ = (name: string) => {
    if (numSelected === 0) {
      toast.warning("Select at least one photo before creating an album.");
      return;
    }
        ...
        {selectMode &&
          <>
            ...
            <FloatButton
              ...
              onClick={() => {
                dispatch(setSelectedPhotos(Object.keys(filteredPhotos)));
              }}
            />
          </>}
